 ...
 */
final class WebhookOutboxAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private WebhookOutboxRepository $webhookOutboxRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'webhook_outbox' => $this->webhookOutboxRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\WebhookOutbox\ModuleException("Record {$idStr} not found for locking.");
            }
