...
final class WebhookOutboxModule implements ModuleInterface
{
    public function name(): string { return 'table-webhook_outbox'; }
    public function table(): string { return 'webhook_outbox'; }
...
SQL;
    ...
    public function status(Database $db, SqlDialect $d): array
    {
        ...
        $expectedIdx = [ 'idx_webhook_created_at', 'idx_webhook_status_scheduled' ];
        if ($d->isMysql()) {
    ...
    public function info(): array
    {
        return [
            ...
            'indexes'     => [ 'idx_webhook_created_at', 'idx_webhook_status_scheduled' ],
            'foreignKeys' => [],
        ];
    }
}
