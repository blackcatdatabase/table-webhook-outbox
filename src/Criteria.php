...
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'event_type', 'payload', 'status', 'retries', 'next_attempt_at', 'created_at', 'updated_at' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'event_type', 'payload', 'status', 'retries', 'next_attempt_at', 'created_at', 'updated_at' ], true);
            $parts = [];
