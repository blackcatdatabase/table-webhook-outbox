...
use BlackCat\Database\Packages\WebhookOutbox\Dto\WebhookOutboxDto as Dto;
use BlackCat\Database\Packages\WebhookOutbox\Mapper\WebhookOutboxDtoMapper as RowMapper;
...
final class WebhookOutboxRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\WebhookOutbox\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\WebhookOutbox\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\WebhookOutbox\Dto\WebhookOutboxDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\WebhookOutbox\Dto\WebhookOutboxDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\WebhookOutbox\Mapper\WebhookOutboxDtoMapper::fromRow($row);
    }
